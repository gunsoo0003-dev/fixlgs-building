...
const CT_LOCAL_UI={
  en:{back:'Back',fieldGuide:'FIELD GUIDE',practical:'PRACTICAL QUESTIONS',reference:'REFERENCE NOTE',realCases:'REAL-WORLD CASES',check:'CHECK',found:'FOUND',action:'ACTION',decision:'DECISION',principle:'CORE PRINCIPLE',final:'FINAL SUMMARY',whatCheck:'WHAT TO CHECK',why:'WHY IT MATTERS',prevent:'PREVENT',ifFound:'IF DISCOVERED',look:'LOOK HERE',compare:'COMPARE',normal:'NORMAL',different:'IF DIFFERENT',next:'NEXT STEP',flow:'CONTRACT FLOW',baseline:'NORMAL BASELINE',variables:'VARIABLES',howTo:'HOW TO CHECK'},
  ja:{back:'戻る',fieldGuide:'FIELD GUIDE · 実務ガイド',practical:'PRACTICAL QUESTIONS · 実務質問',reference:'REFERENCE NOTE · 参考メモ',realCases:'REAL-WORLD CASES · 実例',check:'CHECK · 確認',found:'FOUND · 確認結果',action:'ACTION · 対応',decision:'DECISION · 判断',principle:'CORE PRINCIPLE · 核心原則',final:'FINAL SUMMARY · 最終確認',whatCheck:'WHAT TO CHECK · 確認項目',why:'WHY IT MATTERS · 重要な理由',prevent:'PREVENT · 予防',ifFound:'IF DISCOVERED · 発見時',look:'LOOK HERE · 確認先',compare:'COMPARE · 比較',normal:'NORMAL · 正常',different:'IF DIFFERENT · 不一致時',next:'NEXT STEP · 次の段階',flow:'CONTRACT FLOW · 契約フロー',baseline:'NORMAL BASELINE · 基準状態',variables:'VARIABLES · 取引変数',howTo:'HOW TO CHECK · 確認方法'}
};
function localUi(locale,key){return CT_LOCAL_UI[locale]?.[key]||key;}
...
function LocalizedPartiesVisual({copy,page,locale}){
  const adv=CONTRACT_I18N_V035.advanced?.[locale]?.parties;
  if(!adv) return <Detail copy={copy} page={page} locale={locale}/>;
  const ui=CT_LOCAL_UI[locale]||CT_LOCAL_UI.en;
  const deep=adv.deep, complete=adv.complete;
  const bullets=(items)=><ul className="ctp-bullets">{items.map(x=><li key={x}>{x}</li>)}</ul>;
  const core=page.sections||[];
  return <main className="contract-page ct-parties-page">
    <section className="ctp-hero"><div className="ct-shell"><a className="ct-back" href={`${BASE_PATH}/${locale}/contract`}>{copy.back||ui.back}</a><small>{page.num} · {page.label}</small><h1>{page.title}</h1><h2>{page.question}</h2><p>{page.hero}</p><figure className="ctp-editorial-visual is-hero"><img src={CONTRACT_PARTIES_IMAGES.hero} alt={locale==='ja'?'契約相手・権限・支払先の関係を確認する概要':'Overview of the contracting party, authority and payment relationship'} loading="eager" decoding="async"/></figure></div></section>
    <section className="ctp-firstdeal" data-ct-reveal><div className="ct-shell"><header><small>{ui.fieldGuide}</small><h2>{deep.intro}</h2></header><div className="ctp-firstdeal-list">{deep.cards.map((card,i)=><article key={card.title}><div className="ctp-step-index"><span>{String(i+1).padStart(2,'0')}</span></div><div className="ctp-step-main"><h3>{card.title}</h3><p>{card.body}</p></div><div className="ctp-step-check"><div><small>{ui.check}</small>{bullets(card.points)}</div></div></article>)}</div></div></section>
    <section className="ctp-normal" data-ct-reveal><div className="ct-shell"><header><small>{ui.baseline}</small><h2>{core[0]?.[0]||page.title}</h2><p>{core[0]?.[1]||page.hero}</p></header><div className="ctp-normal-chain">{localChain(locale,'parties').map((x,i)=><div key={x}><span>{String(i+1).padStart(2,'0')}</span><strong>{x}</strong>{i<4&&<b>→</b>}</div>)}</div><figure className="ctp-editorial-visual is-normal-variables"><img src={CONTRACT_PARTIES_IMAGES.normalVariables} alt={locale==='ja'?'本人契約と代理・共有・法人などの変数を比較する概要':'Comparison of direct-owner and agent, co-owner or company transaction variables'} loading="lazy" decoding="async"/></figure></div></section>
    <section className="ctp-variables" data-ct-reveal><div className="ct-shell"><header><small>{ui.whatCheck}</small><h2>{complete.title}</h2><p>{complete.overview}</p></header><div className="ctp-variable-detail">{complete.checks.map((item,i)=><article key={item[0]}><div className="ctp-variable-title"><span>{String(i+1).padStart(2,'0')}</span><div><h3>{item[0]}</h3><p>{item[1]}</p></div></div><div className="ctp-variable-layout"><div className="ctp-variable-check"><small>{ui.check}</small>{bullets(item[2])}</div></div></article>)}</div></div></section>
    <section className="ctp-method" data-ct-reveal><div className="ct-shell"><header><small>{ui.practical}</small><h2>{deep.fieldTitle}</h2><p>{deep.intro}</p></header><figure className="ctp-editorial-visual is-howto"><img src={CONTRACT_PARTIES_IMAGES.howToCheck} alt={locale==='ja'?'所有者・本人・権限・口座を順番に確認する実務フロー':'Practical flow for checking owner, identity, authority and payment account'} loading="lazy" decoding="async"/></figure><ol>{deep.fieldQuestions.map((q,i)=><li key={q}><span className="ctp-how-no">{String(i+1).padStart(2,'0')}</span><div className="ctp-how-body"><h3>{q}</h3></div></li>)}</ol></div></section>
    <section className="ctp-cases" data-ct-reveal><div className="ct-shell"><header><small>{ui.realCases}</small><h2>{locale==='ja'?'実際の状況で確認と対応をつなげます。':'Connect verification and response through real-world scenarios.'}</h2></header><div className="ctp-case-detail">{complete.scenarios.map((x,i)=><article key={x[0]}><div className="ctp-case-head"><span>CASE {String(i+1).padStart(2,'0')}</span><h3>{x[0]}</h3><p>{x[1]}</p></div><div className="ctp-case-body"><div><small>{ui.why}</small><p>{x[2]}</p></div><div><small>{ui.action}</small><p>{x[3]}</p></div></div></article>)}</div></div></section>
    <DeepDive page={page} locale={locale}/><CompleteGuide page={page} locale={locale}/>
    <section className="ctp-principle"><div className="ct-shell"><small>{ui.principle}</small><h2>{core[5]?.[1]||complete.title}</h2></div></section>
    <section className="ctp-final" data-ct-reveal><div className="ct-shell"><header><small>{ui.final}</small><h2>{locale==='ja'?'次へ進む前の最終質問':'Final questions before you move on'}</h2></header><div className="ctp-final-list">{complete.questions.map((q,i)=><article key={q}><span>{String(i+1).padStart(2,'0')}</span><h3>{q}</h3></article>)}</div></div></section>
    <p className="ct-notice ct-shell">{copy.notice}</p><OtherLinks copy={copy} current={page.slug} locale={locale}/>
  </main>;
}

function LocalizedJourney0206({copy,page,locale}){
  const base=CONTRACT_0206_V013[page.slug];
  const adv=CONTRACT_I18N_V035.advanced?.[locale]?.[page.slug];
  if(!base||!adv) return <Detail copy={copy} page={page} locale={locale}/>;
  const ui=CT_LOCAL_UI[locale]||CT_LOCAL_UI.en;
  const deep=adv.deep, complete=adv.complete, core=page.sections||[];
  const bullets=(items)=><ul className="ctj-bullets">{items.map(x=><li key={x}>{x}</li>)}</ul>;
  const chain=localChain(locale,page.slug);
  const nextMap={property:'payment',payment:'terms',terms:'recheck',recheck:'closing',closing:'checklist'};
  const nextPage=copy.pages.find(x=>x.slug===nextMap[page.slug]);
  return <main className={`contract-page ct-journey ct-journey-${page.slug}`}>
    <section className="ctj-hero"><div className="ct-shell"><a className="ct-back" href={`${BASE_PATH}/${locale}/contract`}>{copy.back||ui.back}</a><small>{page.num} · {page.label}</small><h1>{page.title}</h1><h2>{page.question}</h2><p>{page.hero}</p><figure className="ctj-visual is-hero"><img src={base.images.hero} alt={locale==='ja'?`${page.title}の主要確認事項を整理した概要グラフィック`:`Overview graphic for ${page.title}`} loading="eager" decoding="async"/></figure></div></section>
    <section className="ctj-flow" data-ct-reveal><div className="ct-shell"><header><small>{ui.flow}</small><h2>{locale==='ja'?'確認事項を一つの取引フローとしてつなげます。':'Connect the checks into one transaction flow.'}</h2><p>{core[0]?.[1]||page.hero}</p></header><ol>{chain.map((x,i)=><li key={x}><span>{String(i+1).padStart(2,'0')}</span><strong>{x}</strong><p>{core[Math.min(i,core.length-1)]?.[0]||x}</p></li>)}</ol></div></section>
    <section className="ctj-normal" data-ct-reveal><div className="ct-shell"><header><small>{ui.baseline}</small><h2>{core[1]?.[0]||page.title}</h2><p>{core[1]?.[1]||page.hero}</p></header><div className="ctj-chain">{chain.map((x,i)=><div key={x}><span>{String(i+1).padStart(2,'0')}</span><strong>{x}</strong>{i<chain.length-1&&<b>→</b>}</div>)}</div><figure className="ctj-visual is-support"><img src={base.images.support} alt={locale==='ja'?`${page.title}の基準状態と実務上の確認点を比較するグラフィック`:`Comparison graphic for ${page.title}`} loading="lazy" decoding="async"/></figure></div></section>
    <section className="ctj-variables" data-ct-reveal><div className="ct-shell"><header><small>{ui.variables}</small><h2>{complete.title}</h2><p>{complete.overview}</p></header><div className="ctj-variable-list">{complete.checks.map((item,i)=><article key={item[0]}><div className="ctj-row-head"><span>{String(i+1).padStart(2,'0')}</span><div><h3>{item[0]}</h3><p>{item[1]}</p></div></div><div className="ctj-variable-body"><div><small>{ui.whatCheck}</small>{bullets(item[2])}</div></div></article>)}</div></div></section>
    <section className="ctj-missed" data-ct-reveal><div className="ct-shell"><header><small>{ui.fieldGuide}</small><h2>{deep.fieldTitle}</h2><p>{deep.intro}</p></header><div className="ctj-missed-list">{deep.cards.map((card,i)=><article key={card.title}><span>{String(i+1).padStart(2,'0')}</span><h3>{card.title}</h3><p>{card.body}</p>{bullets(card.points)}</article>)}</div></div></section>
    <section className="ctj-method" data-ct-reveal><div className="ct-shell"><header><small>{ui.howTo}</small><h2>{locale==='ja'?'実務ではこの質問を順番に確認します。':'Use these questions in sequence during the transaction.'}</h2></header><ol>{deep.fieldQuestions.map((q,i)=><li key={q}><span className="ctj-how-no">{String(i+1).padStart(2,'0')}</span><div className="ctj-how-main"><h3>{q}</h3></div></li>)}</ol></div></section>
    <DeepDive page={page} locale={locale}/><CompleteGuide page={page} locale={locale}/>
    <section className="ctj-cases" data-ct-reveal><div className="ct-shell"><header><small>{ui.realCases}</small><h2>{locale==='ja'?'状況 → 注意 → 対応まで確認します。':'Follow the situation from risk to response.'}</h2></header><div>{complete.scenarios.map((x,i)=><article key={x[0]}><div><span>CASE {String(i+1).padStart(2,'0')}</span><h3>{x[0]}</h3></div><dl><dt>{ui.check}</dt><dd>{x[1]}</dd><dt>{ui.why}</dt><dd>{x[2]}</dd><dt>{ui.action}</dt><dd>{x[3]}</dd></dl></article>)}</div></div></section>
    <section className="ctj-decision" data-ct-reveal><div className="ct-shell"><header><small>{ui.decision}</small><h2>{locale==='ja'?'次へ進む前に答えられるか確認します。':'Make sure you can answer these before moving on.'}</h2></header><div>{complete.questions.map((q,i)=><article key={q}><span>{String(i+1).padStart(2,'0')}</span><h3>{q}</h3></article>)}</div></div></section>
    <section className="ctj-principle"><div className="ct-shell"><small>{ui.principle}</small><h2>{core[5]?.[1]||complete.title}</h2></div></section>
    <section className="ctj-final" data-ct-reveal><div className="ct-shell"><header><small>{ui.final}</small><h2>{complete.title}</h2><p>{complete.overview}</p></header>{nextPage&&<div className="ctj-next"><div><small>{ui.next}</small><strong>{page.title}</strong></div><a href={`${BASE_PATH}/${locale}/contract/${nextPage.slug}`}><small>{ui.next}</small><strong>{nextPage.num} {nextPage.title}</strong><p>{nextPage.question}</p><b>→</b></a></div>}</div></section>
    <p className="ct-notice ct-shell">{copy.notice}</p><OtherLinks copy={copy} current={page.slug} locale={locale}/>
  </main>;
}
...
function Checklist07({copy,page,locale}){
  ...
  return <main className="contract-page ct7-page">
    ...
    <section className="ct7-usage" data-ct-reveal><div className="ct-shell"><header><small>{d.usage.eyebrow}</small><h2>{d.usage.title}</h2></header><div className="ct7-usage-modes">{d.usage.modes.map(([title,label,body])=><article key={title}><small>{label}</small><h3>{title}</h3><p>{body}</p></article>)}</div><h3 className="ct7-usage-flow-title">{ui.usageFlow}</h3><ol>{d.usage.steps.map(([no,time,body])=><li key={no}><span>{no}</span><strong>{time}</strong><p>{body}</p></li>)}</ol><blockquote>{d.usage.note}</blockquote><figure className="ct7-editorial-visual ct7-documents-image"><img src={`${BASE_PATH}/images/contract/building-contract-07-essential-detail.webp`} alt={locale==='en'?'Using the ESSENTIAL and DETAIL contract checklists together':locale==='ja'?'ESSENTIALとDETAILの2種類の契約チェックリストを併用する場面':'필수 체크리스트와 상세 체크리스트 두 종류의 고급 A4 계약 점검 문서를 함께 사용하는 장면'} loading="lazy" decoding="async"/></figure></div></section>
...
function Mistakes09({copy,page,locale}){
  ...
  return <main className="contract-page ct8-page ct9-page">
    ...
    <section className="ct8-content" data-ct-reveal><div className="ct-shell"><header className="ct8-content-intro"><small>{d.content.eyebrow}</small><h2>{d.content.title}</h2><p>{d.content.intro}</p></header><figure className="ct89-editorial-image"><img src={`${BASE_PATH}/images/contract/building-contract-09-skipped-checks.webp`} alt={locale==='en'?'Mistake pattern caused by skipping verbal-agreement documentation, document comparison and rechecking':locale==='ja'?'口頭合意の文書化・資料照合・再確認など重要手順を省略するミスの流れ':'부동산 계약 과정에서 구두합의, 문서대조, 재확인 등 중요한 절차가 생략되는 실수 흐름'} loading="lazy" decoding="async"/></figure><div className="ct8-editorial-stack">{d.content.sections.map((section)=><article className="ct8-editorial" key={section.no}><div className="ct8-editorial-meta"><span>{section.no}</span><small>{section.label}</small></div><h3>{section.title}</h3><div className="ct8-editorial-copy">{section.paragraphs.map(p=><p key={p}>{p}</p>)}</div><div className="ct8-editorial-flow">{section.flow.map((item,i)=><div key={item}><span>{String(i+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></article>)}</div></div></section>

    <section className="ct8-scan ct9-scan-unified" data-ct-reveal><div className="ct-shell"><header className="ct8-scan-intro"><small>{d.scan?.eyebrow||'MISTAKE SCAN · 계약 실수 점검'}</small><h2>{d.scan?.title||'내가 반복하는 생략부터 찾아냅니다.'}</h2><p>{d.scan?.body||d.content.transition?.body}</p></header><div className="ct8-scan-groups">{d.groups.map((group,gIndex)=><section className="ct8-scan-group" key={group.code}><header><span>{String(gIndex+1).padStart(2,'0')}</span><div><small>{group.code}</small><h3>{group.name||group.ko}</h3></div></header><ol>{group.items.map(([mistake,why,block],i)=><li key={mistake}><span className="ct8-signal-no">MISTAKE {String(i+1).padStart(2,'0')}</span><h4>{mistake}</h4><div className="ct8-signal-detail"><p><strong>{locale==='ko'?'WHY IT HAPPENS · 왜 생기나':locale==='ja'?'WHY IT HAPPENS · なぜ起こるか':'WHY IT HAPPENS'}</strong>{why}</p><p><strong>{locale==='ko'?'BLOCK IT · 이렇게 차단':locale==='ja'?'BLOCK IT · 防ぐ方法':'BLOCK IT'}</strong>{block}</p></div></li>)}</ol></section>)}</div><figure className="ct89-editorial-image ct89-editorial-image-scan"><img src={`${BASE_PATH}/images/contract/building-contract-09-failure-control-path.webp`} alt={locale==='en'?'Comparison of a skipped-check failure path and a verification-based control path':locale==='ja'?'確認省略による失敗経路と検証中心の管理経路を比較する場面':'같은 부동산 거래에서 확인 생략의 실수 경로와 검증 중심의 통제 경로를 비교한 장면'} loading="lazy" decoding="async"/></figure>
...
export default function ContractPage({locale='ko',initialView=[]}){
  ...
  if(page&&page.slug==='parties') return locale==='ko'?<PartiesPrototype copy={copy} page={page} locale={locale}/>:<LocalizedPartiesVisual copy={copy} page={page} locale={locale}/>;
  if(page&&CONTRACT_0206_V013[page.slug]) return locale==='ko'?<Journey0206 copy={copy} page={page} locale={locale}/>:<LocalizedJourney0206 copy={copy} page={page} locale={locale}/>;
