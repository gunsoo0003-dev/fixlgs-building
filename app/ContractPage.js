...
function buildLocalizedJourneyData(locale,page){
  ...
  return {
    ...
    flow:{eyebrow:'CONTRACT FLOW',title:localText(locale,'Connect each check as one transaction flow.','各確認を一つの取引フローとしてつなげます。'),lead:core[0]?.[1]||page.hero,steps:chain.map((x,i)=>[String(i+1).padStart(2,'0'),x,cycle(core,i,[])[1]||cycle(checks,i,[])[1]||page.hero])},
    normal:{eyebrow:'NORMAL BASELINE',title:localText(locale,'Start from the normal baseline before reading exceptions.','例外を見る前に正常な基準状態から確認します。'),lead:core[1]?.[1]||page.hero,chain:normalChain,result:localText(locale,'When the sequence connects without unexplained gaps, move to the next verification.','説明できないずれがなくつながれば次の確認へ進みます。')},
    variables:{eyebrow:'VARIABLES',title:complete.title,lead:complete.overview,items:Array.from({length:4},(_,i)=>{const c=cycle(checks,i,[]);return {title:c[0]||cycle(questions,i,page.title),situation:c[1]||complete.overview,check:c[2]||[genericCheck],why:cycle(deep.cards,i,{}).body||deep.intro,prevent:cycle(c[2]||[],0,genericCheck),respond:cycle(questions,i,genericAction),decision:genericAction};})},
    missed:{eyebrow:'MISSED POINT',title:localText(locale,'These are the points most easily skipped during a real transaction.','実際の取引で特に抜けやすい確認点です。'),body:deep.intro,points:Array.from({length:5},(_,i)=>[cycle(questions,i,cycle(checks,i,[])[0]||page.title),cycle(checks,i,[])[1]||deep.intro]),prevention:questions.slice(0,5)},
    method:{eyebrow:'HOW TO CHECK',title:localText(locale,'Use the same verification order in practice.','実務でも同じ確認順序を使います。'),lead:deep.intro,steps:Array.from({length:5},(_,i)=>{const c=cycle(checks,i,[]);return {title:c[0]||cycle(questions,i,page.title),where:c[1]||deep.intro,compare:cycle(c[2]||[],0,genericCheck),normal:localText(locale,'The current evidence matches the expected condition.','現在の資料が想定条件と一致しています。'),ifDifferent:localText(locale,'Identify the cause and impact of the difference.','差異の原因と影響を特定します。'),actions:c[2]||[genericCheck],result:cycle(questions,i,genericAction)};})},
    mismatch:{eyebrow:'WHEN THINGS DO NOT MATCH',title:localText(locale,'If the expected condition and actual condition differ, verify before proceeding.','想定条件と実際の状態が違う場合は、進む前に確認します。'),examples:Array.from({length:3},(_,i)=>{const x=cycle(scenarios,i,[]);return [x[1]||cycle(questions,i,''),x[2]||genericCheck,x[3]||genericAction];}),process:questions.concat([genericAction]).slice(0,6)},
    ...
    decision:{title:localText(locale,'Choose the next action from what you verified.','確認結果から次の行動を決めます。'),items:[
    ...
    principle:core[5]?.[1]||localText(locale,'Do not move money until the required condition is verifiably complete.','必要条件の完了を確認できるまで資金を動かしません。'),
    final:{title:localText(locale,'Final check before moving on','次へ進む前の最終確認'),lead:complete.overview,items:Array.from({length:base.final.items.length},(_,i)=>{const c=cycle(checks,i,[]);return [c[0]||cycle(questions,i,page.title),c[1]||deep.intro];})}
  };
}
